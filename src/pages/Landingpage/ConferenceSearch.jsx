...
function ConferenceSearch(props) {
	...
	const removeFromFollowed = (idev) => {
		axios
			.delete(
				apiPath + "/Users/" + authCtx.userid + "/Events",
	...
	const onSearchChange = (eve) => {
		setFilteredConferences(
			allConferences.filter((conf) => {
				return conf.title
					.toLowerCase()
					.includes(eve.target.value.toLowerCase());
			})
		);
	};
				...
				<>
					{!isFullWidth && (
						<div className={classes.tabHolder}>
							<Button
								className={
									myEventsChosen ? classes.buttonactive : ""
								}
					...
					<div className={classes.adaptiveFlex}>
						{(!myEventsChosen || isFullWidth) && (
							<div className={classes.search}>
								<div
									className={`${classes.searchWrapper} ${classes.flex}`}
								>
									<Search
