...
const NavigationBar = () => {
	...
	const notificationButtonStyle = noOfNewNotifications > 0 ? { background: "#cc3300" } : null;
	const noOfNotificationInButton = noOfNewNotifications > 0 ? `(${noOfNewNotifications})` : '';
	const notificationButtonName = `Notifications${noOfNotificationInButton}`;
	const notificationButtonMobClassName = `navigation-bar__option__name${noOfNewNotifications > 0 ? '--new-notif': ''}`
	const notificationMobIconStyle = noOfNewNotifications > 0 ? { color: "red" } : null;

	const menuMobile = (
		<Menu className="navigation-bar__dropdown" style={{ backgroundColor: "#1E3D58", border: "white"}}>
			...
			<Menu.Item key="notifications" className="navigation-bar__menu-item">
				<Link to="/notifications" onClick={onNotificationsClick}>
					<div className="navigation-bar__option-container">
						...
						<div className={notificationButtonMobClassName}>{notificationButtonName}</div>
					</div>
				</Link>
			</Menu.Item>
			...
			<Menu.Item key="logout" className="navigation-bar__menu-item">
				<Link to="/logout">
					<div className="navigation-bar__option-container">
						<ImportOutlined/>
	...
	function handleSwMessage(e) {
		...
		setNoOfNewNotifications((oldNoOfNewNotifications) => oldNoOfNewNotifications + 1);
	...
	useEffect(() => {
		...
		const currentUnseenNotifications = localStorage.getItem('noOfNewNotifications');
		...
		if (!currentUnseenNotifications) {
			...
			localStorage.setItem('noOfNewNotifications', noOfFetchedNotifications - currentUnseenNotifications);
			setNoOfNewNotifications(noOfFetchedNotifications - currentUnseenNotifications);
		}
	...
	return (
		<div className="navigation-bar">
			<div className="navigation-bar__logo">
				<Link to="/">
					<img src='/calendar-64.png' alt="ctIcon" className="navigation-bar__logo__img" />
				</Link>
				<div className="navigation-bar__logo__name">
			...
			{windowWidth > mobileWidth ? (
				<div className="navigation-bar__options">
					...
					<Link to="/notifications">
						<Tooltip visible={newNotification} title={newNotification} color="#cc3300" onClick={onNotificationsClick}>
							<Button icon={<BellFilled/>} style={notificationButtonStyle}>{notificationButtonName}</Button>
						</Tooltip>
					</Link>
					<Link to="/admin">
				...
				<div className="navigation-bar__options">
					<Dropdown
						...
						>
						<Tooltip visible={newNotification} title={"New Event changes!"} color="#cc3300" placement="left">
